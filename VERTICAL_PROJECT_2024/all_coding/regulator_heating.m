...
%% using the PI to determine its quality
%% this differs when working with input that can only increase or only decrese the temperature.
%% in this particular section, the heater can only increase the temperature

run('vesna_credentials_json.m'); 
vesna = vesna;
...
while true
    ...
    if currentTemperature > desiredTemperature && heaterValue == 100
        ...
        controlSignal_heater_1 = Kp_heater * error_temp - Ki_heater * error_temp 
