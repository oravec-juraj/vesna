...
desiredTemperature = 25; % Example: 25 degrees Celsius
...
while true
    ...
    fansValue = data.fans.value;
    
    % Calculate error
    error_temp =  currentTemperature - desiredTemperature;

    % Update integral term
    %integralTerm_fans = integralTerm_fans + error_temp;
    
    % Humidifier Control (directly affecting temperature)
    if currentTemperature > desiredTemperature && fansValue == 100
        disp('Cannot dicrease temperature with fans at 100.');
    else
        controlSignal_fans_1 = Kp_fans * error_temp - Ki_fans * error_temp %za druhe error_temp bolo integralTerm_fans
        %controlSignal_fans_1 - treba menit to znamienko jak Miska pisala aby to workovalo
        scaling_factor = 100.0 / max(1, abs(controlSignal_fnas_1))

        controlSignal_fans_2 = controlSignal_fans_1 * scaling_factor
        controlSignal_fans_3 = max(0, min(100, controlSignal_fans_2))

        % Send control signal to humidifier through IoT Cloud
        vesna.upload("fnas", round(controlSignal_fans_3)); % Round to integer

        % Display values sent to the humidifier
        disp(['Control Signal to Fans: ', num2str(round(controlSignal_fansr_3))]);
    end
    % Store data for plotting
    time(end+1) = now; % append current time
    temperature(end+1) = currentTemperature;
    control_signal(end+1) = controlSignal_fans_3;
