...
void setup() 
{
  ...
  TCA9548A(1);
  if (!bme1.begin(0x44)) {
    Serial.println("Could not find a valid sht30 sensor on bus 2, check wiring!");
    while (1);
  }
  Serial.println(); 
 ...
 //2
  ...
  //4
    TCA9548A(4);
  if (!bme1.begin(0x44)) {
    Serial.println("Could not find a valid fs400 sensor on bus 2, check wiring!");
    while (1);
  }
  Serial.println(); 
...
int sensorValue = analogRead(15);
 ...
 printValues1(sht30, 1);
  printValues2(lps35hw, 2);
  printValues3(sht4, 3);
  printValues4(sht31, 4);
