...
export default function Temperature({ navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('https://thingspeak.com/channels/1632859/fields/1.json?results=1');
   ...
   useEffect(() => {
     const fetchData = async () => {
       try {
         const res = await fetch('https://thingspeak.com/channels/1635834/fields/3.json?results=1');
         const json = await res.json();
         setData1(json);
       } catch (error) {
         console.log(error);
  ...
  return (
    <>
     ...
     <StatusBar style="light" />
      ...
      <WebView source={{ uri: 'https://thingspeak.com/channels/1632859/charts/1?bgcolor=%23ffffff&color=%23d62020&dynamic=true&results=60&type=line&update=15' }} 
        ...
        <View style={{flexDirection:"row",  height: 40, }}>
          ...
          <View style={{flex:0.9}}>
            <FlatList
              ...
              renderItem={({ item }) => (
                <Text style={styles.t_text}>{parseFloat(item.field1).toFixed(2)} °C</Text>
              )}
            />
          </View>
        </View>
      ...
      <View style={{flex: 0.1}}>
        <View style={{flexDirection:"row",  height: 55}}>
          ...
          <FlatList
              data ={data1.feeds}
              keyExtractor={({ field3 }, index) => field3}
              renderItem={({ item }) => (
                <Text style={styles.text}>Actuator {parseFloat(item.field3).toFixed()} %</Text>
              )}
            />
          </View>
      ...
      <WebView source={{ uri: 'https://thingspeak.com/channels/1635834/charts/3?bgcolor=%23ffffff&color=%23d62020&dynamic=true&results=60&type=line&update=15' }} 
      scalesPageToFit={true}
